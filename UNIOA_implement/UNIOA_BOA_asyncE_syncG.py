...
class BOA_UNIOA_asyncE_syncG(NatureOpt):
    ...
    def __call__(self):
        ...
        while not self.stop:
            # Opt temp_x(t+1) and evaluation
            temp_X, temp_X_Fit = self.Opt_X.boa_but_asyncE(func=self.fitness_function, old_X=X, old_X_Fit=X_Fit, x_g=x_g, z=z, w1=self.w1, w2=self.w2, lb_x=self.lb_x, ub_x=self.ub_x)
            # Selection
