...
class GOA_UNIOA_asyncE_syncG(NatureOpt):
    ...
    def __call__(self):
        ...
        # Optimizing
        while not self.stop:
            # Oopt x(t+1) and Evaluate
            temp_X,temp_X_Fit = self.Opt_X.goa_but_asyncE(func=self.fitness_function, old_X=X, old_X_Fit=X_Fit,old_x_g = x_g, z=z, lb_x=self.lb_x, ub_x=self.ub_x, w1=self.w1, w2=self.w2)
            # Selection
