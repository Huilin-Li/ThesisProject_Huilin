...
class BA_orig_syncE_syncG(NatureOpt):
    ...
    def __call__(self):
        ...
        while not self.stop:
            ...
            Fitness_temp = self.Evaluate_X(X=Sol_temp)
            # selection
            for i in range(self.M):
                if ((Fitness_temp[i] <= Fitness[i]) and (np.random.rand() > A)):
                    Sol[i] = Sol_temp[i] # final new generated pop
